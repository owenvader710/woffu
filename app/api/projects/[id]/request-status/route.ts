...
function extractProjectId(req: NextRequest, context?: { params?: { id?: string } }) {
  ...
  const m = path.match(/\/api\/projects\/([^/]+)\/request-status\/?$/);
  return m?.[1];
}
...
export async function POST(req: NextRequest, context: { params?: { id?: string } }) {
  try {
    ...
    if (!isUuid(projectId)) return NextResponse.json({ error: `Invalid project id: project_id` }, { status: 400 });
    ...
    // กันส่งซ้ำ: ถ้ามี pending อยู่แล้วของ user คนนี้
    ...
    if (pending && pending.length > 0) {
      const res = NextResponse.json(
        { ok: true, message: "Already pending (รอหัวหน้าอนุมัติคำขอเดิมก่อน)" },
        { status: 200 }
      );
